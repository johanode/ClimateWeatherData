...
def filter_time(df, ts, time_period, idx='Date', col='Value'):
    ...
    if col is not None:
        return df.set_index(idx).loc[time_filter[0]:time_filter[-1],col]
    else:
        return df.set_index(idx).loc[time_filter[0]:time_filter[-1]]
...
def TAS(station, ts, time_period='y'):
    ...
    filtered_weather_values = filter_time(weather_data, ts, time_period)
...
def DTR(station, ts, time_period='m'):
    ...
    # Filter based on failure time and time period
    temperature_min = filter_time(weather_data, ts, time_period)
    ...
    # Filter based on failure time and time period
    temperature_max = filter_time(weather_data, ts, time_period)
...
def PRRN(station, ts, time_period='y'):
    ...
    # Filter based on failure time and time period
    precip_values = filter_time(weather_data, ts, time_period)
    ...
    # Filter based on failure time and time period
    precip_types = filter_time(weather_data, ts, time_period)

    # Join precipitation values with type of precipitation
    precip_data = precip_values.to_frame().join(precip_types.rename('Type'))

    # Sum for different types of precipitation
    precip_sum = precip_data.groupby(by=['Type']).sum()
     
    # Define rain types
    valid_types = ['regn', 'duggregn', 'regnskurar']
    # Check if types in data
    idx = [ty for ty in valid_types if ty in precip_sum.index]
    # Sum up the different types
    value = precip_sum.loc[idx, 'Value'].sum()
...
def PRSN(station, ts, time_period='y'):
    ...
    # Snow types
    valid_types = ['snowfall', 'kornsnö', 'snöblandat regn', 'snöbyar',
               'byar av snöblandat regn', 'snöhagel']
    # Check if types in data
    idx = [ty for ty in valid_types if ty in precip_sum.index]
    # Sum up the different types
    value = precip_sum.loc[idx, 'Value'].sum()
...
def SuperCooledPR(station, ts, time_period='y'):
    ...
    # Cooled rain types
    valid_types = ['underkyld nederbörd']
...
def PRSNmax(station, ts, time_period='y'):
    ...
    # Max for different types of precipitation
    precip_max = precip_data.groupby(by=['Type']).max()
     
    # Snow types
    valid_types = ['snowfall', 'kornsnö', 'snöbyar'] # 'snöblandat regn', 'byar av snöblandat regn', 'snöhagel'
    # Check if types in data
    idx = [ty for ty in valid_types if ty in precip_max.index]
    # Sum up the different types
    value = precip_max.loc[idx, 'Value'].max()
...
def SfcWind(station, ts, time_period='y'):
    ...
    # Filter based on failure time and time period
    filtered_weather_values = filter_time(weather_data, ts, time_period, idx='Date (UTC)')
